<x-app-layout>
    ...
    <div class="w-4/5 bg-gray-100 m-5">
        <div class="flex justify-between">
            <span class="text-blue-800 uppercase text-3xl font-black">
                {{__('Courses')}}
            </span>

            <a class="px-4 py-2 bg-blue-500 rounded text-white shadow-md" href="{{ route('courses.create') }}">
                {{__('Create Course')}}
            </a>
        ...
        <div class="w-full flex mb-2 h-52 bg-white p-2">
            ...
            <div class="relative w-full">
                ...
                <div class="font-thin text-gray-500">
                    Created by:<span class="font-black text-black mr-2 ml-1">{{ $course->users }}</span>
                    Created on:<span class="font-black text-black ml-1">{{ $course->created_at }}</span>
                </div>
                <div class="font-thin text-gray-500">
                    {{ $course->description }}
                </div>
                <div class=" absolute bottom-0"><span class="mr-2">
                        {{ $course->level->name }}
                    </span><span>{{ count($course->enrolledUsers) }} Enrolled</span></div>
                <span class="absolute top-2 right-12 px-2 border-2  text-sm rounded {{ ($course->status->name == 'published') ? 'bg-green-100 border-green-700 text-green-700' : (($course->status->name == 'draft') ? 'bg-yellow-100 border-yellow-700 text-yellow-700' : 'bg-red-100 border-red-700 text-red-700') }}">
                    {{ $course->status->name }}
                </span>
                ...
                <div x-data="{ show:false}" @click.away="show = false" class="absolute top-2 right-2">
                    ...
                    <div x-show="show" class="absolute border-2 border-black-600 w-50 right-0">
                        <a class="bg-gray-100 hover:bg-gray-400 block text-left px-3 leading-7" href="{{ route('courses.destroy',$course) }}">
                            {{ __('Delete') }}
                        </a>
                        <a class="bg-gray-100 hover:bg-gray-400 block text-left px-3 leading-7" href="{{ route('courses.edit',$course)}}">
                            {{__('Edit') }}
                        </a>
                        <form action="{{ route('courses.status',$course)}}" method="post">
                            @foreach($statuses as $status)
                            @csrf
                            @if($status->name != $course->status->name)
                            <button class="bg-gray-100 w-full hover:bg-gray-400 text-left px-3" type="submit" name="status_id" value="{{ $status->id }}">
                                {{ $status->name }}
                            </button>
                            @endif
                            @endforeach
                        </form>
                        <a class="bg-gray-100 hover:bg-gray-400 block text-left px-3 leading-7" href="{{ route('courses.status',$course)}}">
