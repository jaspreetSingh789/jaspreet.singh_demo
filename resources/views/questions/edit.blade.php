<x-app-layout>
    <section class="flex-auto m-5">
        ...
        <div class="pt-5">
            ...
            <a href="{{ route('courses.tests.edit',[$course,$test]) }}" class="font-bold text-xl">
                {{ $test->name }}
            </a>
            <strong class="px-2 font-bold text-xl ">></strong>
            <span class="font-bold text-xl">
                {{ __('Edit') }} {{ $question->name }}
            </span>
        </div>
        ...
        <main class="w-full mt-5 border border-gray-50 p-6 bg-white relative">
            <form method="post" action="{{ route('courses.tests.questions.update',[$course,$test,$question]) }}" class="mt-5">
                ...
                <div class="mb-6">
                    ...
                    <a class="bg-blue-300 text-white rounded ml-5 px-4 py-2 hover:bg-blue-200 hover:text-white border-blue-300 " href="{{ route('courses.tests.edit',[$course,$test]) }}">
                        {{ __('Cancel') }}
                    </a>
                </div>
            </form>
